...
Std_ReturnType gpio_pin_direction_initialize(const pin_configration *pin)
{
    ...
    {
        case output:
            *TRIS_REGESTERS[pin->port]&=~((uint8)1<<pin->pin);
            break;
        case input:
            *TRIS_REGESTERS[pin->port]|=((uint8)1<<pin->pin);
            break;
...
{
    Std_ReturnType RET = (Std_ReturnType)0x01;
    if(((void*)0)==pin || ((void*)0)==direction )
    {
       RET = (Std_ReturnType)0x00;
    }
    else
    {
        ...
        *direction = ((*TRIS_REGESTERS[pin->port]>>pin->pin)&(uint8)1);
...
Std_ReturnType gpio_pin_write_logic(const pin_configration *pin,logic_t logic)
{
    ...
    else
    {
        ...
        {
            case low :
                *LAT_REGESTERS[pin->port]&=~((uint8)1<<pin->pin);
                break;
...
Std_ReturnType gpio_pin_read_logic(const pin_configration *pin,logic_t *logic)
{
    ...
    {
        *logic = ((*PORT_REGESTERS[pin->port]>>pin->pin)&(uint8)1);
    }
     return RET;
}
...
Std_ReturnType gpio_port_toggle (port_index_t port)
{
    ...
    {
        *LAT_REGESTERS[port]^= 0xff ;
    }
    return RET;
}
# 283 "MCAL_layer/GPIO/hal_gpio.c"
Std_ReturnType gpio_pin_initialize(const pin_configration *pin)
{
    Std_ReturnType RET = (Std_ReturnType)0x01;
    if(((void*)0)==pin)
